...
INPUT_JSONL = THIS_DIR / "PIE_RQ1.jsonl"
OUT_JSONL = THIS_DIR / "PIE_RQ1_with_scores.jsonl"
...
def compute_crystalbleu_single(ref: str, hyp: str, language: str) -> float:
    try:
        ...
        return float("nan")
    try:
        val = crystal_BLEU([ref], [hyp], language=language)
        return float(val[0] if isinstance(val, (list, tuple)) else val)
    except Exception:
        return float("nan")

def compute_codebertscore_batch(refs: List[str], hyps: List[str], language: str, batch_size: int = 64) -> List[float]:
    try:
...
def load_pairs_rows(p: Path, limit=None) -> List[Dict[str, Any]]:
    ...
    with p.open("r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                continue
            if not all(k in obj for k in ("id", "golden_code", "generated_code", "score", "surface_similarity")):
                continue
            rows.append({
                "id": obj["id"],
                "golden_code": obj["golden_code"],
                "generated_code": obj["generated_code"],
                "score": obj["score"],
                "surface_similarity": obj["surface_similarity"],
            })
    return rows
...
def main():
    ...
    data = load_pairs_rows(INPUT_JSONL, limit=LIMIT)
    if not data:
    ...
    for i, row in enumerate(data):
        ...
        ss_val = float(row["surface_similarity"]) if isinstance(row["surface_similarity"], (int, float)) else float("nan")
        try:
            gt_val = float(row["score"])
        except Exception:
            gt_val = float("nan")
        ...
        out_rows.append({
            ...
            "golden_code": ref,
            "generated_code": hyp,
            "score": gt_val if not math.isnan(gt_val) else None,
            "surfaceSim": ss_val if not math.isnan(ss_val) else None,
            "codebleu": None if math.isnan(cbleu) else cbleu,
            "crystalbleu": None if math.isnan(crys) else crys,
            "codebertscore": None if math.isnan(cbert_i) else cbert_i,
            "codescore": None if math.isnan(cscore) else cscore,
            "abs_surfaceSim_minus_score": None if math.isnan(abs_diff) else abs_diff,
        })
