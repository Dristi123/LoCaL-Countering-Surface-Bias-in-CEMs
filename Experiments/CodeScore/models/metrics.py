...
#             mcc_value = list(mcc.values())[0] if mcc else 0.0  
#         else:  
            
#             mcc_value = mcc  
#         return {self.prefix + "_mcc": mcc_value}
class MCCMetric(Metric): 
        def __init__(self, prefix: str = "", **kwargs):  
            super().__init__()  
            self.prefix = prefix  
            filtered_kwargs = {k: v for k, v in kwargs.items() if k != 'task'}  
            self.mcc_metric = MatthewsCorrCoef(task="binary", **filtered_kwargs)  
        
        def update(self, preds, target):  
            self.mcc_metric.update(preds, target)  
        
        def compute(self):  
            mcc = self.mcc_metric.compute()  
