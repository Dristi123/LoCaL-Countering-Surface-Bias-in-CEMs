...
#print("MCCMetric is from:", inspect.getfile(MCCMetric))
...
class UnifiedMetric(CometModel):
    ...
    def init_metrics(self):
        ...
        if self.hparams.word_level_training:
            # Train and Dev MCC
            self.train_mcc = MCCMetric(task="binary", num_classes=2, prefix="train")
            print("MCC type::::::::::::::", type(self.train_mcc))
            self.val_mcc = nn.ModuleList(
                [
                    MCCMetric(task="binary", num_classes=2, prefix=d)
    ...
    def validation_step(
        ...
        if dataloader_idx == 0:
            ...
            if self.word_level:
                logits_1d = torch.softmax(logits, dim=1)[:, 1]
                self.train_mcc.update(logits_1d, targets)
            ...
            if self.word_level:
                logits_1d = torch.softmax(logits, dim=1)[:, 1]
                self.val_mcc[dataloader_idx - 1].update(logits_1d, targets)
    ...
    def validation_epoch_end(self, *args, **kwargs) -> None:
        ...
        #print("MCCMetric source file:;;;;;;;;;;;;;", inspect.getfile(models.metrics.MCCMetric))
        if self.word_level:
            #print("MCC type::::::::::::::", type(self.train_mcc))
            print("MCC class at runtimeeeeeeeeeee:", self.train_mcc.__class__.__name__)
